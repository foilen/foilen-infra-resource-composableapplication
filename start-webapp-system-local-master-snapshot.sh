...
docker run -ti \
  ...
  /plugins application dns machine mariadb postgresql unixuser website

# Create release
./create-local-release.sh
cp build/libs/foilen-infra-resource-composableapplication-master-SNAPSHOT.jar $FOLDER_PLUGINS_JARS
